					...
					public void run() {
						for (Pet pet : pets) {
							...
							if (pInfo.getRareNum() > 4 && pInfo.getRareNum() % 2 == 1) {
								...
								try {
									FileUtil.appendTxt("尝试购买超级稀有: " + pInfo + "\n", PropUtil.getProp("success_buy_path"));
								} catch (Exception e1) {
									e1.printStackTrace();
								}
								...
								if (b) {
									String str = String.format(user.getName() + " 超级稀有购买成功！交易时间：%s, petId:%s, 售价：%s, 等级:%s %s %s, 休息时间：%s ",
											TimeUtil.now(TimeUtil.TARGET_1), pet.getId(), pet.getAmount(), pet.getRareDegree(), pet.getGeneration()
													+ "代", pInfo.getRareNum() + "稀", pet.getCoolingInterval());
